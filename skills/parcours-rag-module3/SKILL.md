...
10. **Fidélité à Mastra**. Avant d'écrire du code Mastra, consulte le skill `mastra` pour les APIs à jour. S'il n'est pas installé dans le workspace courant, installe-le d'abord pendant le bootstrap (`npx skills add mastra-ai/skills --skill mastra -y`). Ne devine pas d'API — vérifie dans `node_modules/@mastra/core/dist/docs/references/`.
