...
| Pilotage | Skill linéaire à étapes validées, exécuté par l'agent de codage de chaque participant |
| Vérification | Agent inspecte fichiers/sortie. Hints progressifs si bloqué. |
| Rythme | Facilitateur pilote la discussion collective entre chaque étape |
...
design/module-3-design.md          ← ce document (source de vérité)
         ...
         │    ├── steps/01-setup.md             (une étape = une unité de progression validée)
         │    ├── steps/02-ingest.md
         │    ├── ...
         ...
         │    - 1 slide d'intro par étape
         │    - 1 slide de debrief par étape
         │
...
Ces 5 LO sont la boussole. Chaque étape du skill doit contribuer à au moins un LO. La vérification d'une étape doit être observable (l'agent peut la constater).
...
| 1 | **Étape 1** — Setup projet Mastra + corpus | 20 min | Agent-piloté, puis debrief 5 min | LO1 |
| 2 | **Étape 2** — Ingestion & chunking | 30 min | Agent-piloté, puis debrief 10 min | LO1, LO4 |
| 3 | **Étape 3** — Embeddings & index vectoriel | 25 min | Agent-piloté, puis debrief 5 min | LO1, LO2 |
| 4 | **Pause** | 10 min | — | — |
| 5 | **Étape 4** — Retrieval & requête | 25 min | Agent-piloté, puis debrief 5 min | LO2 |
| 6 | **Étape 5** — Génération avec contexte + citations | 30 min | Agent-piloté, puis debrief 10 min | LO3 |
| 7 | **Étape 6** — Test sur questions d'éval + analyse de failles | 20 min | Agent-piloté + discussion | LO4, LO5 |
...
## 5. Spécification des 6 étapes

Pour chaque étape, le skill fournit à l'agent du participant :
...
### Étape 2 — Ingestion & chunking (30 min)
...
### Étape 3 — Embeddings & index (25 min)
...
### Étape 5 — Génération avec citations (30 min)
...
### Étape 6 — Éval & analyse de failles (20 min)
...
| 5 | **Template Étape 1** | Starter Mastra avec **un simple agent de chat sans mémoire**. Point de départ minimal, permet de valider « ça marche » avant d'attaquer le RAG. |
...
L'implémentation existante (`AlbertAPIGateway` dans assistant-rh) gère déjà le chat via `resolveLanguageModel`. Pour l'Étape 3 il faut ajouter :
...
| Un participant plante à l'Étape 1, bloque toute sa trajectoire | Le skill doit pouvoir sauter en aval si l'Étape 1 est validée par fichiers manuellement copiés ; docker image fallback |
...
| Participants finissent en avance | « Side quest » optionnelle par étape (ex: Étape 2 → essayer un chunker sémantique) |
...
3. Rédiger en détail les Étapes 1 et 2 (les plus critiques pour la dynamique de démarrage)
4. Construire le corpus et l'index de référence, identifier les 5 questions d'éval
5. Rédiger en détail les Étapes 3 → 6
6. Rédiger le hint ladder complet par étape
