...
Ces observations alimentent directement : (1) la rédaction des hints et vérifications CP2–CP6, (2) les débriefs collectifs, et (3) le pont vers Module 4.

## 1. Profil du corpus — trouvailles inattendues

| Attendu (design doc §CP2) | Observé | Conséquence |
|---|---|---|
| ≥500 chunks | **55 chunks** (chunking page-par-page, token 500 / overlap 50) | L'exit criterion CP2 doit être revu à la baisse. Proposer « ≥40 chunks » (plancher sûr) ou plutôt « chaque PDF produit ≥1 chunk avec `text`/`source`/`page` ». |
| Guide « Mots de passe ANSSI » présent | **Absent** | La question canonique du design doc (« Quelles sont les règles d'hygiène des mots de passe selon l'ANSSI ? ») est en réalité **hors corpus**. Parfaite matière pour le profil « piège » de CP6 — voir Q3 ci-dessous. |
...
- **Conséquence pédagogique** : c'est une faille *observable à l'œil nu*. Parfait matériau CP2/CP6. À mentionner explicitement dans les pièges pédagogiques CP2.
...
**Leçon** : un seuil à 0.55–0.60 ne séparerait PAS les questions hors-corpus des questions transverses. C'est le cœur de la discussion CP5/CP6 sur les garde-fous.
...
- **Statut** : happy path fidèle. C'est la démo qui rassure les participants au CP4. À noter pour CP4 : le top-5 est plus grand que le nombre de chunks pertinents — phénomène récurrent sur ce petit corpus.
...
- **Statut** : démo de la synthèse multi-chunk réussie. Idéal pour CP5. Les chunks dupliqués passent inaperçus côté génération.
...
## 5. Synthèse des 3+ failles observables en CP6
...
L'exit criterion CP6 (« ≥3 failles observées avec exemples concrets ») est largement atteignable — un participant attentif en trouvera 4–5 sans effort.

## 6. Implications pour les PR de détail (CP1–CP6)

- **PR #5 (CP1+CP2)** : revoir l'exit criterion CP2 (≥40 chunks, pas ≥500). Mentionner explicitement le piège « chunks quasi-dupliqués par page » et la pollution d'en-tête/pied.
- **PR #6 (CP3–CP5)** : CP3 stats attendues ~55 chunks / 55 vecteurs en dimension 1024. CP4 : utiliser Q1 et Q2 comme questions de smoke test. CP5 : la question canonique « mots de passe ANSSI » fonctionne MAL — à remplacer par Q1 Zero Trust pour la démo en CP5.
- **PR #7 (hint ladder)** : pas d'impact direct ; les hints restent du même format.
- Le **design doc** (§CP2 « Exit » et §CP5 « Exit ») a deux erreurs factuelles à corriger en douceur — mais ce n'est pas dans la portée de cette PR.
